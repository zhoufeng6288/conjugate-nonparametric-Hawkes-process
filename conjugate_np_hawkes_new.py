...
#########################################################################################################
    ...
    def MF(self, num_gq_mu, num_gq_phi, num_pre_mu, num_pre_phi, num_iter):
        ...
        for iteration in range(num_iter):
            ...
            Pij_flat=[]
            for i in range(N): 
                mu_ti=lamda_1_mu*expit(c_ii[i])*np.exp((a_ii[i]-c_ii[i])/2)
                tau_ij_start=sum(num_Pij_row[:i])
                tau_ij_end=sum(num_Pij_row[:i+1])
                phi_ti=lamda_1_phi*expit(c_ij[tau_ij_start:tau_ij_end])*np.exp((a_ij[tau_ij_start:tau_ij_end]-c_ij[tau_ij_start:tau_ij_end])/2)
                intensity_total=mu_ti+np.sum(phi_ti)
                P[i][i]=mu_ti/intensity_total
                P_i_j=phi_ti/intensity_total
                P[i][i-len(phi_ti):i]=P_i_j
            ...
            mu_mean_mf = lamda_mu_mean*expit(mean_g_mu)
            phi_mean_mf = lamda_phi_mean*expit(mean_g_phi)
            logl_train = self.loglikelihood_discrete_phi_mu(self.points_hawkes, mu_mean_mf, phi_mean_mf, self.T_phi, self.T)
            logl_test = self.loglikelihood_discrete_phi_mu(self.points_hawkes_test, mu_mean_mf, phi_mean_mf, self.T_phi, self.T_test)

            # record
            g_mu_mean_list.append(mean_g_mu)
            g_mu_cov_list.append(cov_g_mu)
            g_phi_mean_list.append(mean_g_phi)
            g_phi_cov_list.append(cov_g_phi)
            alpha_mu_list.append(alpha_mu)
            alpha_phi_list.append(alpha_phi)
            logl_train_list.append(logl_train)
            logl_test_list.append(logl_test)
        return g_mu_mean_list, g_mu_cov_list, g_phi_mean_list, g_phi_cov_list, alpha_mu_list, alpha_phi_list, logl_train_list, logl_test_list